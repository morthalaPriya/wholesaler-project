...
export default function ResetPassword({ onSelectMethod, onBackToLogin }) {
  function handleMobileClick() {
    alert('Mobile verification is currently unavailable. Please verify via Email.');
  }
  ...
  return (
    <div className="h-screen w-full flex flex-col md:flex-row font-sans bg-[#FAF9FF] overflow-hidden">
      ...
      <div className="md:w-1/2 w-full h-full flex items-center justify-center p-6 md:p-12 shrink-0">
        <div className="w-full max-w-[440px] h-[320px] bg-white p-6 md:p-8 rounded-xl shadow-2xl shadow-slate-500/50 border border-slate-100 flex flex-col justify-between">
          
          <div className="text-center">
            <h2 className="text-lg font-semibold text-slate-900 tracking-tight">Reset Password</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-auto">
            ...
            <button
              ...
              className="group p-5 rounded-2xl border border-slate-200 bg-white hover:border-slate-300 hover:shadow-md transition-all flex flex-col items-center text-center cursor-pointer"
            >
              <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center mb-3 group-hover:bg-slate-300/80 transition-colors">
                <img src={emailIcon} alt="Email" className="w-5 h-5 object-contain" />
              </div>
              <span className="text-xs font-semibold text-slate-800 mb-0.5">Verify via Email</span>
              <span className="text-[10px] text-slate-400 font-normal">Receive OTP by email</span>
            </button>
            <button
              type="button"
            ...
            >
              <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center mb-3 group-hover:bg-slate-300/80 transition-colors">
                <img src={mobileIcon} alt="Mobile" className="w-5 h-5 object-contain" />
              </div>
              <span className="text-xs font-semibold text-slate-800 mb-0.5">Verify via Mobile</span>
              <span className="text-[10px] text-slate-400 font-normal">Receive OTP by SMS</span>
            </button>
